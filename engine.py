...
def play_game(player, entities, game_map, message_log, game_state, con, panel, constants):
    ...
    while not libtcod.console_is_window_closed():
        ...
        if spellbook_index is not None and previous_game_state != GameStates.PLAYER_DEAD and \
            ...
            if game_state == GameStates.SHOW_SPELLBOOK:
                if spell.spell.mana_cost > player.fighter.mana:
                    message_log.add_message(
                        Message('Not enough mana to cast {}'.format(spell.name), libtcod.yellow))
                else:
                    player_turn_results.extend(
                        player.spellbook.cast_spell(spell, entities=entities, fov_map=fov_map,
        ...
        if game_state == GameStates.SPELL_TARGETING:
            ...
            # refacto.
            spell_use_results = spell_targeting_resolution(targeting_obj, target_mode, player, game_map, fov_map,
                                                           entities, action, left_click, right_click)
